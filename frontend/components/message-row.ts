    ...
    const editButton = this.canEdit ? (this.editing ?
                                       ...
                                       html`
                                         <button
                                           ...
                                           type="button" @click=${() => this.editing = true}>
    ...
    return html`
      <p style="overflow-wrap: break-word;" class="flex items-center">
        ...
        <textarea
          ...
          class="bg-transparent border-0 p-0 focus:text-orange-200 transition-colors focus:outline-0 focus:border-0 focus:ring-0 focus:underline leading-none cursor-text min-w-0 grow me-auto"
          ?disabled=${!this.editing} .value=${live(this.message)} wrap="soft"></textarea>

        ${editButton}

        <!-- copy -->
        <button class="p-0 text-emerald-500 hover:text-emerald-600 cursor-pointer me-2" type="button" @click=${() => navigator.clipboard.writeText(this.message)}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4">
