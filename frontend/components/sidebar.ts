...
export default class Sidebar extends StyledElement {
  ...
  render() {
    ...
    const groups = this.groups.map(group => {
      ...
      const groupMembers = html`<p class="text-gray-300 text-sm">${group.members.join(", ")}</p>`;

      if (isGroupRecipient && group.id === (this.currentRecipient as { Group: number }).Group) {
        return html`
          <p
            class="px-3 py-1 outline outline-1 outline-orange-600 bg-orange-950 last:border-b-0 first:rounded-t last:rounded-b"
            >
            <span class="flex items-center">${group.name} ${groupActions}</span>
            ${groupMembers}
          </p>
        `;
      } else {
        return html`
          <button
            class="hover:bg-gray-600 cursor-pointer px-3 py-1 block text-left w-full border-b border-gray-600 last:border-b-0 transition-colors"
